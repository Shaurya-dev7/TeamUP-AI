  ...
  async function toggleFollow() {
    ...
    if (!followed) {
      ...
      if (res.ok) {
        ...
        setFollowersList(prev => prev.concat([{ id: userId, username: 'you', display_name: 'You' }]));
      }
    } else {
      const res = await fetch('/api/follow', {
        method: 'DELETE',
                ...
                <div className="mt-4">
                  ...
                  <div className="flex gap-2">
                    {followersList.slice(0, 6).map(f => (
                      <div key={f.id || f.username} className="flex flex-col items-center text-center w-16">
                        {/* Avatar removed */}
                        <div className="text-xs">{f.username || f.id || 'Unknown'}</div>
                      </div>
                    ))}
                  </div>
                </div>
